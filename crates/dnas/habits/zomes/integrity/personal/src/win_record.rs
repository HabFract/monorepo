...
pub struct WinRecord {
    pub data: HashMap<ActionHashB64, WinData>,
}
...
pub type WinData = HashMap<String, WinDateValue>;
...
pub fn validate_create_wins(
    ...
    // for (_action_hash, win_data) in wins.data.iter() {
    //     for (date_string, _value) in win_data.iter() {
    //         if !DateString::is_valid_date_format(&date_string.0) {
    //             return Ok(ValidateCallbackResult::Invalid(
    //                 format!("Invalid date format: {}", date_string.0)
    //             ));
    //         }
    //     }
    // }
