...
function getOrbitScaleImg(scale: Scale) {
  switch (scale) {
    case Scale.Atom:
      return {
        src: "assets/orbits/scale-outlines-1.png",
        alt: "Subatomic Orbit",
      };
    case Scale.Sub:
    ...
    case Scale.Astro:
      return {
        src: "assets/orbits/scale-outlines-3.png",
        alt: "Astronomic Orbit",
      };
  }
}
  ...
  return (
    <div className="orbit-vis">
      <div>
        <img
          className="w-24"
          alt={getOrbitScaleImg(scale)?.alt}
          src={getOrbitScaleImg(scale)?.src}
        />
        <Progress
          className={scale.toLowerCase()}
          progress={getOrbitScalePercent(scale)}
          color="dark"
        />
        <label>
          <div className="pt-1 mt-2 text-center w-full">Scale</div>
        </label>
      </div>
    </div>
  );
