...
const RadioGroup: React.FC<RadioGroupProps> = ({ id, name, options, labelValue, required, withInfo, disabled, direction }: RadioGroupProps) => {
  const [selected, setSelected] = useState<string>(options[0])

  return (
    <WithLabel id={id} labelValue={labelValue} required={required} withInfo={withInfo}>
      <div data-name={name} className={direction == "vertical" ? "flex max-w-md flex-col gap-2" : "flex max-w-md flex-row gap-6"}>
...
export const RadioGroupField: React.FC<{ field: any, form: any, props: RadioGroupProps }> = ({
  field,
  form: { touched, errors, setFieldValue: _ },
  ...props
}: any) => {
  const { name, labelValue, options, direction, value: __, id, required, onBlur: ___ } = props;
  return (
    <>
      <RadioGroup
        name={name}
