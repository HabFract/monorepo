...
import { GetOrbitHierarchyDocument, OrbitHierarchyQueryParams } from '../../graphql/generated';
import { client } from '../../graphql/client';

const usePrefetchNextLevel = async(queryParams: OrbitHierarchyQueryParams, bypass: boolean = false) => {
  const gql: ApolloClient<NormalizedCacheObject> = (await client) as ApolloClient<NormalizedCacheObject>;
  try {
    console.log('client :>> ', queryParams);
    await gql.query({
