...
import { currentSphereAtom, currentSphereHasCachedNodesAtom, currentSphereHashesAtom } from '../state/sphere';
...
export const useRedirect = (bypass?: boolean) => {
  ...
  const isSameSphere = (params?.landingSphereId && sphere.actionHash && params.landingSphereId !== sphere.actionHash) || (params?.currentSphereAhB64 && sphere.actionHash && params.currentSphereAhB64 !== sphere.actionHash);
  ...
  useEffect(() => {
    ...
    console.log('orbitEdges.length > 0 && !sphereHasCachedOrbits :>> ', orbitEdges.length > 0 && !sphereHasCachedOrbits);
