...
const ImageUpload = ({
  ...
  const uploadButton = (
    <div className="absolute z-10 top-4 left-4 text-gray-500 ">
      <div className="upload-img-btn absolute z-10 cursor-pointer bg-gray-500 opacity-75 text-white rounded-full p-2 transition-all duration-300">
        Upload
        <span className="rounded-xl text-primary bg-secondary ml-2">
    ...
    >
      {imageUrl && (custom || showSeeded) ? (
        <div className="relative w-full h-full overflow-hidden">
          ...
          <div className="absolute w-8 h-4 top-1 left-1">{uploadButton}</div>
        </div>
      ) : (
