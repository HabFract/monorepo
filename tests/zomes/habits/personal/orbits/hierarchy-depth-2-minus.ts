  ...
  test("Orbit Hierarchy Happy Path - Depth <= 2", async (t) => {
    await runScenario(async (scenario) => {
      ...
      try {
        ...
        const childOrbits = await createOrbitChildren(
          callZomeAlice,
        ...
        t.equal(
          orbitHierarchyResponse2.children.length,
          2,
          "orbit 1 has two children"
        );

        const childOrbitHashes = childOrbits.map((child) =>
          encodeHashToBase64(child.entryHash)
        );
        const hierarchyChildrenContents = orbitHierarchyResponse2.children.map(
          (child) => child.content
        );
        const secondHasFirst = hierarchyChildrenContents.includes(
          childOrbitHashes[0]
        );
        const secondHasSecond = hierarchyChildrenContents.includes(
          childOrbitHashes[1]
        );
        t.ok(
          secondHasFirst && secondHasSecond,
          "and those children have their hashes as content."
        );
