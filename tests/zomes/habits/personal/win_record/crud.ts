...
import { anOrbit, setupSphere } from "../orbits/utils";
  ...
  test("Win Record CRUD - Happy path 1 (single creates)", async (t) => {
    await runScenario(async (scenario) => {
      ...
      try {
        ...
        t.ok(
          hash1 && hash2,
          "A sphere was created with an orbit linked to it,"
        );
        ...
        t.ok(
          createWinRecordResponse2,
          "and a multiple win record was created,"
        );
        // Then the WinRecord was created
      } catch (e) {
        t.ok(null);
  ...
  test("Win Record CRUD - Happy path 2  (batch creates)", async (t) => {
    await runScenario(async (scenario) => {
      ...
      try {
        ...
        t.ok(
          getOrbitWinRecordForYearMonthResponse,
          "A query was made to get win records for an orbit in a particular YYYY.MM,"
        );
        t.equal(
          getOrbitWinRecordForYearMonthResponse.length,
          0,
          "and the response was an array of the correct length (0)."
        );
        ...
        const unbucketedWinData = {
          "31/12/2020": { multiple: [true, false, true] },
          "02/01/2021": { multiple: [false, true, false] },
        };
        ...
        await pause(pauseDuration);
        t.ok(
          createWinRecordsResponse,
          "Multiple bucketed win records were created,"
        );
        t.equal(
          createWinRecordsResponse?.length,
          2,
          "and they were returned in an array of records of the correct length,"
        );
        ...
        t.equal(
          createdWinRecordEntryRecords[0].entry.winData["31/12/2020"]
            .multiple[0],
          true,
          "and they returned the correct win records (1 of 2),"
        );
        t.equal(
          createdWinRecordEntryRecords[1].entry.winData["02/01/2021"]
            .multiple[1],
          true,
          "and they returned the correct win records (2 of 2),"
        );
        t.equal(
          createdWinRecordEntryRecords[0].entry.winData["02/01/2021"],
          undefined,
          "and they returned a win record without the other bucketed record (1 of 2),"
        );
        t.equal(
          createdWinRecordEntryRecords[1].entry.winData["31/12/2020"],
          undefined,
          "and they returned a win record without the other bucketed record (2 of 2),"
        );
        ...
        const getResponse3EntryRecord0 = new EntryRecord<WinRecord>(
          getOrbitWinRecordForYearMonthResponse2[0]
        );
        t.ok(
          getOrbitWinRecordForYearMonthResponse2,
          "A query was made to get win records for an orbit in the first YYYY.MM,"
        );
        t.equal(
          getOrbitWinRecordForYearMonthResponse2.length,
          1,
          "and the response was an array of the correct length,"
        );
        t.deepEqual(
          getResponse3EntryRecord0.entry,
          createdWinRecordEntryRecords[0].entry,
          "and the returned response included the correct WinRecord."
        );
        ...
        const getResponse3EntryRecord1 = new EntryRecord<WinRecord>(
          getOrbitWinRecordForYearMonthResponse3[0]
        );
        t.ok(
          getOrbitWinRecordForYearMonthResponse3,
          "A query was made to get win records for an orbit in the second YYYY.MM,"
        );
        t.equal(
          getOrbitWinRecordForYearMonthResponse3.length,
          1,
          "and the response was an array of the correct length,"
        );
        t.deepEqual(
          getResponse3EntryRecord1.entry,
          createdWinRecordEntryRecords[1].entry,
          "and the returned response included the correct WinRecord."
        );
        // Then the WinRecords were returned
        ...
        const winDataUpdate = {
          orbitEh: encodeHashToBase64(hash2),
          winData: { "31/12/2020": { single: true } },
        };
        const updateInput = {
        ...
        t.ok(
          updateWinRecordResponse,
          "An update was made to a batch created WinRecord using its action hash,"
        );
        const updateWinRecordEntryRecord = new EntryRecord<WinRecord>(
          updateWinRecordResponse
        );
        t.ok(
          updateWinRecordEntryRecord,
          "and the response was a singular WinRecord record,"
        );
        t.deepEqual(
          updateWinRecordEntryRecord.entry,
          winDataUpdate,
