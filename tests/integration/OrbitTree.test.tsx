...
vi.mock("../../ui/src/state/orbit", async (importOriginal) => {
  ...
  return {
    ...actual,
    getOrbitNodeDetailsFromIdAtom: (id) => {
      return { testId: "getOrbitNodeDetailsFromIdAtom"}
    },
    getOrbitNodeDetailsFromEhAtom: (id) => {
      return { testId: "getOrbitNodeDetailsFromEhAtom"}
    },
    getOrbitIdFromEh: () => ({ testId: "getOrbitIdFromEh"}),
  }
})
vi.mock("../../ui/src/state/win", async (importOriginal) => {
  const actual = (await importOriginal()) as any;
  return {
    ...actual,
    calculateCurrentStreakAtom: (id) => {
      return { testId: "calculateCurrentStreakAtom"}
    },
    calculateLongestStreakAtom: (id) => {
      return { testId: "calculateLongestStreakAtom"}
    },
  }
})
...
vi.mock("../../ui/src/state/store", async (importOriginal) => {
  ...
  const atomMappings = {
    ...
    calculateCurrentStreakAtom: 2,
    calculateLongestStreakAtom: 4,
    currentSphereOrbitNodeDetailsAtom: currentMockSphereNodes,
